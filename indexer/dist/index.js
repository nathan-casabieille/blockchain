...
const ARTIFACTS_DIR = path_1.default.join(__dirname, '../../contracts/artifacts/contracts');
// --- Database Setup ---
let db;
...
async function setupContractListeners(complianceAddr, tokenAddr, oracleAddr, dexAddr) {
    const provider = new ethers_1.ethers.JsonRpcProvider(RPC_URL);
    // Read ABIs
    const readAbi = (name, file) => {
        const p = path_1.default.join(ARTIFACTS_DIR, `${file}.sol/${name}.json`);
    ...
    const complianceContract = new ethers_1.ethers.Contract(complianceAddr, complianceAbi, provider);
    const tokenContract = new ethers_1.ethers.Contract(tokenAddr, tokenAbi, provider);
    const oracleContract = new ethers_1.ethers.Contract(oracleAddr, oracleAbi, provider);
    const dexContract = new ethers_1.ethers.Contract(dexAddr, dexAbi, provider);
    // 1. Compliance Events
